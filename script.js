...
// "idle" - ожидание
// "cooking" = готовка
// "ready" - кофе готов, но пока что не забран
...
function makecoffee(name, price, element) {
  ...
  balance.value = price; //balance.value = balance.value
    ...
    progressBar.style.opacity = readyPercent + "%";
    })
...
function createCoin(nominal) {
  ...
  let changeContainer = document.querySelector(".coffee-change-mt-2");
